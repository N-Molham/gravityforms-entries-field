...
// plugin master file
define( 'GFEF_MAIN_FILE', __FILE__ );

// plugin DIR
define( 'GFEF_DIR', plugin_dir_path( GFEF_MAIN_FILE ) );

// plugin URI
define( 'GFEF_URI', plugin_dir_url( GFEF_MAIN_FILE ) );

// localization text Domain
define( 'GFEF_DOMAIN', 'gf-entries-field' );

require_once GFEF_DIR . 'includes/classes/Singular.php';
require_once GFEF_DIR . 'includes/helpers.php';
require_once GFEF_DIR . 'includes/functions.php';
...
class Plugin extends Singular
{
	...
	public function load_view( $view_name, $args = null )
	{
		...
		$__template_path = GFEF_DIR . 'views/' . $__view_name . '.php';
		if ( !file_exists( $__template_path ) )
		{
			// file not found!
			wp_die( sprintf( __( 'Template <code>%s</code> File not found, calculated path: <code>%s</code>', GFEF_DOMAIN ), $__view_name, $__template_path ) );
		}
		 ...
		 */
		do_action_ref_array( 'gfef_load_template_before', [ &$__template_path, $__view_name, $args ] );
		 ...
		 * @return string
		 */
		require apply_filters( 'gfef_load_template_path', $__template_path, $__view_name, $args );
		 ...
		 * @param string $__view_name
		 */
		do_action( 'gfef_load_template_after', $__template_path, $__view_name, $args );
	}
	...
	{
		load_plugin_textdomain( GFEF_DOMAIN, false, dirname( plugin_basename( GFEF_MAIN_FILE ) ) . '/languages' );
	}
	...
	public function autoloader( $class_name )
	{
		...
		$class_path = GFEF_DIR . 'includes' . DIRECTORY_SEPARATOR . 'classes' . str_replace( [
				__NAMESPACE__,
				'\\',
